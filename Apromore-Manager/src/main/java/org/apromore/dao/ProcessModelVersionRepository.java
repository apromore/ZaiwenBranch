...
public interface ProcessModelVersionRepository extends JpaRepository<ProcessModelVersion, Integer>, ProcessModelVersionRepositoryCustom {
    ...
    /*Find the process model version by id*
    * @param id
    * return processModelVersion we found or null*/
    @Query("SELECT pmv FROM ProcessModelVersion pmv WHERE pmv.id = ?1")
    ProcessModelVersion getProcessModelVersionById(Integer id);
    ...
    /**
     * Find the process model version for the process model version identifier.
     * @param pmvId the pmv id.
     * @return processModelVersion we found or null.
     */
//    @Query("SELECT pmv FROM ProcessModelVersion pmv WHERE pmv.id = ?1")
//    ProcessModelVersion getCurrentProcessModelVersion(Integer pmvId);
