...
public interface ProcessRepository extends JpaRepository<Process, Integer>, ProcessRepositoryCustom {
    ...
    //*Finds the process with process branch Id
    // *@param branchId the branch id of process
    //@return the process
    // *//
    @Query("SELECT p FROM Process p WHERE p.id = ?1 AND p.folder is null")
    Process findUniqueById(Integer id);
