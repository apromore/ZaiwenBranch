...
public class ManagerPortalEndpoint {
    ...
    private WorkspaceService workspaceSrv;
       private ChangePropagationService changePropagationSrv;
    ...
    public ManagerPortalEndpoint(final DeploymentService deploymentService, final PluginService pluginService,
                                 ...
                                 final SecurityService secSrv, final WorkspaceService wrkSrv, final UserInterfaceHelper uiHelper, final ChangePropagationService changePropagationSrv
    ) {
        ...
        this.workspaceSrv = wrkSrv;
               this.changePropagationSrv = changePropagationSrv;
    ...
    public JAXBElement<UpdateProcessOutputMsgType> updateProcess(@RequestPayload final JAXBElement<UpdateProcessInputMsgType> req) {
        ...
        //for update in propagated side
        ResultType updateResultForPropagatedSide = new ResultType();
        res.setUpdatedResultForPropagatedSide(updateResultForPropagatedSide);
        ...
        try {
            ...
            Version originalVersion = new Version(editType.getOriginalVersionNumber());
            Version currentVersion = new Version(editType.getCurrentVersionNumber());

            //get the index for determine if propagate changes
            int propagationIndex = payload.getPropagationIndex();
            ...
            procSrv.updateProcess(editType.getProcessId(), editType.getProcessName(), editType.getOriginalBranchName(), editType.getNewBranchName(),
                    currentVersion, originalVersion, secSrv.getUserByName(editType.getUsername()), Constants.LOCKED, natType, canonisedProcess);

            //on the condition that we consider change propagation
            if(propagationIndex==1){

                //add code for the change propagation function in the following ...
                String username = payload.getEditSession().getUsername();

                //set the 'ProcessVersionIdType' of the original process variant
                ProcessVersionIdType originalProcessVariant = new ProcessVersionIdType();
                originalProcessVariant.setProcessId(editType.getProcessId());
                originalProcessVariant.setBranchName(editType.getOriginalBranchName());
                originalProcessVariant.setVersionNumber(editType.getOriginalVersionNumber());

                //set the 'ProcessVersionIdType' of the changed process variant
                ProcessVersionIdType changedProcessVariant = new ProcessVersionIdType();
                changedProcessVariant.setProcessId(editType.getProcessId());
                changedProcessVariant.setBranchName(editType.getNewBranchName());
                changedProcessVariant.setVersionNumber(editType.getCurrentVersionNumber());

                //set the new branch name of merged model, we set it equal to the new branch name of variant
                String newBranchNameForMergedModel = editType.getNewBranchName();

                //set the new version number of merged model, we set it equal to the new version of variant
                Version newVersionForMergedModel = currentVersion;

                changePropagationSrv.changePropagationFromGtoCG(username,originalProcessVariant,
                        changedProcessVariant,  newBranchNameForMergedModel, newVersionForMergedModel, natType);

            }

            updateResultForPropagatedSide.setCode(0);
            updateResultForPropagatedSide.setMessage("");
            ...
            result.setCode(-1);
            result.setMessage(ex.getMessage());
            //for exception on updating process at propagation side
            updateResultForPropagatedSide.setCode(-1);
            updateResultForPropagatedSide.setMessage(ex.getMessage());
        }
